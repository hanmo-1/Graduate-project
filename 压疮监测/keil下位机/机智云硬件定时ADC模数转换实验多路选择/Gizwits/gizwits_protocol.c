...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuePressure_signal != cur->valuePressure_signal)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuePressure_signal Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagPressure_signal = 1;
            ret = 1;
        }
    }
    if(last->valueTemperature_signal != cur->valueTemperature_signal)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueTemperature_signal Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagTemperature_signal = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    /*** Handle the value segment ***/
		for(int i=0;i<144;i++)
		{
			if(gizwitsProtocol.waitReportDatapointFlag.flagPressure_signal)
			{
					devStatusTmp.valuePressure_signal = exchangeBytes(gizY2XFloat(Pressure_signal_RATIO,  Pressure_signal_ADDITION, dataPoints->valuePressure_signal[i]));     
					gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuePressure_signal,Pressure_signal_LEN);
					byteFieldOffset += Pressure_signal_LEN;
			}
			if(gizwitsProtocol.waitReportDatapointFlag.flagTemperature_signal)
			{
					devStatusTmp.valueTemperature_signal = exchangeBytes(gizY2XFloat(Temperature_signal_RATIO,  Temperature_signal_ADDITION, dataPoints->valueTemperature_signal[i]));     
					gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueTemperature_signal,Temperature_signal_LEN);
					byteFieldOffset += Temperature_signal_LEN;
			}
			if(gizwitsProtocol.waitReportDatapointFlag.flagHumidity)
			{
					devStatusTmp.valueHumidity = exchangeBytes(gizY2XFloat(Humidity_RATIO,  Humidity_ADDITION, dataPoints->valueHumidity[i]));     
