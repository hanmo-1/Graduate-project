...
% Last Modified by GUIDE v2.5 04-Nov-2020 10:51:44
...
s5 = instrfind('Type','serial','Port','COM5','Tag','');
% 获取参数值
...
elseif (isempty(s4) && get(handles.port,'Value')==4 )        % 如果端口4存在，存创建端口 
    ...
    set(handles.edit1,'String','打开成功！');
    set(handles.edit1,'backgroundcolor',[0 1 0]);
elseif (isempty(s5) && get(handles.port,'Value')==5 )        % 如果端口4存在，存创建端口 
    s = serial('COM5');                   % 创建串口号
    if(get(handles.botelv,'Value')==1)    % 设置串口参数
        s.BaudRate = 1200;                   
    elseif(get(handles.botelv,'Value')==2) 
...
% --- Otherwise, executes on mouse press in 5 pixel border or over port.
